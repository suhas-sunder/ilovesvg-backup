// app/routes/img-to-svg-batch.tsx
import * as React from "react";
import type { Route } from "./+types/img-to-svg-batch";
import {
  json,
...
export function meta({}: Route.MetaArgs) {
  const title = "i🩵SVG  -  Batch (server, in-memory, ZIP)";
  const description =
    "Batch-convert up to 100 images to SVG with Potrace on the server (RAM only). Same presets, optional edge preprocessor, single ZIP download.";
  return [
    { title },
...
/* ========================
   Action: Batch Potrace (RAM-only)
   - Accepts up to 100 files (field name "files")
   - Optional server-side "Edge" preprocessor via sharp
   - Returns a ZIP (application/zip) with .svg files
======================== */
export async function action({ request }: ActionFunctionArgs) {
  const uploadHandler = createMemoryUploadHandler();
  const form = await parseMultipartFormData(request, uploadHandler);

  // ---- Read settings (shared across all files) ----
  ...
  if (files.length > 100) {
    return json(
      { error: "Please upload 100 files or fewer." },
      { status: 400 }
    );
  }

  // Optional sharp under ESM (CJS require)
  let sharp: any = null;
  if (preprocess === "edge") {
    try {
  ...
  // Reusable converter
  async function convertOne(blob: File) {
    ...
    const name = sanitize(`${basename(blob.name)}.svg`);
    return { name, svg: finalSVG };
  }

  // Concurrency window (RAM-friendly & fast)
  const LIMIT = 6; // tune as needed for your server
  const results: { name: string; svg: string }[] = [];
  for (let i = 0; i < files.length; i += LIMIT) {
    const slice = files.slice(i, i + LIMIT);
    const part = await Promise.all(slice.map(convertOne));
    results.push(...part);
  }
  ...
  for (const r of results) {
    const base = r.name.replace(/\.svg$/i, "");
    const count = used.get(base) ?? 0;
    used.set(base, count + 1);
    if (count > 0) r.name = `${base}-${count + 1}.svg`;
  }

  // Build ZIP in memory
  for (const r of results) {
    zip.file(r.name, r.svg);
  }
...
export default function BatchPage() {
  ...
  function onPick(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const capped = [...files, ...imgs].slice(0, 100);
    setFiles(capped);
  ...
  const buttonDisabled = isServer || !hydrated || busy || files.length === 0;
      ...
      <div className="max-w-[1180px] mx-auto px-4 pt-6 pb-12">
        <header className="text-center mb-2">
          <h1 className="inline-flex items-center gap-2 text-[34px] font-extrabold leading-none m-0">
            ...
            <span className="text-slate-400 text-xl ml-2">Batch</span>
          </h1>
          <p className="mt-1 text-slate-600">
            Upload up to 100 images. Same presets & settings. One click → ZIP of
            SVGs.
          </p>
        </header>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm overflow-hidden min-w-0">
            ...
            <div
              ...
              onDrop={onDrop}
              onClick={() => document.getElementById("files-inp")?.click()}
            ...
            >
              <div className="text-sm text-slate-600">
                Click, drag & drop, or paste up to 100 PNG/JPEG images
              </div>
              <input
                id="files-inp"
                type="file"
            ...
            {/* Selected files list */}
            {files.length > 0 && (
              <div className="mt-2 rounded-lg border border-[#dae6ff] bg-[#f7faff] p-3">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <button
                    type="button"
                    className="px-2 py-1 rounded-md border border-[#d6e4ff] bg-[#eff4ff] hover:bg-[#e5eeff]"
                    onClick={clearFiles}
                  >
                    Clear
                  </button>
                </div>
                <ul className="mt-2 max-h-48 overflow-auto text-sm text-slate-700 space-y-1">
                  {files.map((f, i) => (
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm min-w-0">
            <h2 className="m-0 mb-3 text-lg text-slate-900">Summary</h2>
            {files.length === 0 ? (
              <p className="text-slate-600 m-0">
                Drop files on the left. You’ll get a single ZIP of SVGs using
                the current settings.
              </p>
            ) : (
              <div className="space-y-2">
                <div className="text-slate-700 text-sm">
                  Ready to convert <strong>{files.length}</strong> file
                  {files.length === 1 ? "" : "s"} • {prettyBytes(totalBytes)}
                </div>
                <p className="text-slate-600 text-sm m-0">
                  Conversion is RAM-only; results aren’t stored. For large
                  batches, we process a few at a time for speed & stability.
                </p>
              </div>
            )}
          </div>
